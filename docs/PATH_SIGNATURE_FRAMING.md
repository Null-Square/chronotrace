...
This is not literally the classical continuous path signature. The distinction matters: path-signature coefficients are iterated integrals of a path, while the ChronoTrace finite interactions are empirical differences among complete nonlinear training operators. The structural analogy is useful, but they should not be equated without a derivation connecting a specific training limit to the classical object.
...
- truncation produces information loss;
- higher degree can become necessary when lower-order terms cannot separate candidate paths.
...
Important correction: because this identity is algebraically equivalent to the two-candidate nearest-signature comparison, observing `chi >= 1` for a realized misclassification is a **diagnostic restatement**, not independent evidence that a mechanism predicted the failure. Its scientific use is to decompose *which direction* the omitted interaction contaminated, not to validate the theory by itself.

The nontrivial empirical questions are instead whether the contamination repeatedly has a structured form across independent instances—for example whether it is concentrated on same-prefix tail swaps, and whether independently measured prefix-conditioned interaction geometry explains that structure.
...
## Probe complexity is not decoding complexity

A fixed-order interaction basis can be obtained with polynomially many stage-map probes.

If all ordered interactions through degree K are measured, the number of ordered stage-map extensions scales as

`sum_(r=1..K) P(N,r) = O(N^K)`

for fixed K.

This is a statement about **basis/probe complexity**.

It does **not** imply that recovering the best consistent permutation from that basis is automatically polynomial. There are `N!` complete chronologies, and exhaustive nearest-signature enumeration is still factorial.

Even a pairwise model can induce a globally constrained ranking problem: pair orientations cannot be chosen independently because they must correspond to one consistent total order. General weighted ordering / tournament problems can be combinatorially difficult.

ChronoTrace should therefore report two distinct scaling quantities:

1. **probe complexity:** how many candidate training-stage executions are needed to construct the forensic representation;
2. **inference complexity:** how much computation is needed to recover a consistent chronology from that representation.

A future scalable decoder may use:

- beam search over prefixes;
- branch-and-bound using lower-order separation bounds;
- integer / constraint optimization for consistent pair orientations;
- partial-order recovery when total order is underdetermined;
- prefix-adaptive pruning that only evaluates higher-order interactions for ambiguous branches.

The scientific advantage of a low interaction order survives even if exact combinatorial decoding remains hard, but the two claims must not be conflated.

## Prefix-adaptive inference as a path search

Suppose low-order evidence identifies a small set of plausible first-stage prefixes. Rather than measuring every complete chronology, one can conceptually:

1. maintain a beam of plausible prefixes;
2. evaluate interactions among remaining stages at the states induced by those prefixes;
3. extend only prefixes whose chronology remains observationally compatible with the endpoint;
4. stop when the remaining pair orientations are robustly identified or when ambiguity cannot be resolved at the permitted interaction order.

With a fixed beam width B and pair probes at each prefix depth, the number of stage-map probes is roughly polynomial in N, of order `O(B N^3)` in a simple implementation. This is not a theorem of optimality and does not remove worst-case combinatorial ambiguity; it is a concrete route to testing whether empirical training histories are much easier than worst-case permutation search.

For three stages there is essentially no computational advantage: conditioning on the first stage and explicitly measuring both remaining tail orders nearly touches the complete histories. This is why a meaningful efficiency claim for degree-3 reconstruction requires `N >= 4`.
...
7. Report probe complexity and inference complexity separately.
8. Treat failure of a low-order truncation as information about path complexity, not automatically as failure of the broad chronology hypothesis.
