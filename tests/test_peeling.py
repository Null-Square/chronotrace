...
from chronotrace.geometry.peeling import (
    decode_last_stage_from_forward_reachable_sets,
...
def test_forward_reachable_residual_recovers_all_histories_without_inversion() -> None:
    stages, _, base, maps, _ = _affine_problem()
    basis = _interaction_basis(stages, maps, base, max_degree=3)
    histories = tuple(permutations(stages))
    ...
    for history in histories:
        ...
        degree_three_correct += int(degree_three == history)
        ...
        for candidate_last in stages:
            ...
            predecessor_words[candidate_last] = words
            endpoints = []
            matrix, bias = maps[candidate_last]
            for word in words:
                predecessor = _prediction(word, basis, base, max_degree=3)
                endpoints.append(matrix @ predecessor + bias)
            reachable_sets[candidate_last] = tuple(endpoints)

        decision = decode_last_stage_from_forward_reachable_sets(target, reachable_sets)
        assert decision.identifiable
        assert decision.stage is not None
        assert decision.predecessor_index is not None
        predecessor = predecessor_words[decision.stage][decision.predecessor_index]
        decoded = predecessor + (decision.stage,)
        reachable_correct += int(decoded == history)

    assert degree_three_correct == 18
    assert reachable_correct == 24


def test_forward_reachable_residual_reports_collision_as_nonidentifiable() -> None:
...
def test_forward_reachable_tolerance_requires_margin_above_noise_budget() -> None:
    target = np.array([0.0, 0.0])
    reachable_sets = {
        "A": (np.array([0.01, 0.0]),),
        "B": (np.array([0.015, 0.0]),),
    }

    strict = decode_last_stage_from_forward_reachable_sets(target, reachable_sets)
    guarded = decode_last_stage_from_forward_reachable_sets(
        target,
        reachable_sets,
        tie_tolerance=0.006,
    )
