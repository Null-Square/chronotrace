#!/usr/bin/env python3
"""Aggregate the frozen label-blind four-seed confirmation suite."""
...
def main() -> None:
    ...
    if lock.get("confirmation_version") != (
        ...
        raise RuntimeError("confirmation aggregate requires the frozen v2 lock")
    if lock.get("freeze_status") != (
        "frozen_before_any_heldout_confirmation_output_after_label_blind_audit"
    ):
        raise RuntimeError("confirmation aggregate lock status drift")
    ...
    for result in results:
        if result.get("result_version") != (
            ...
            raise RuntimeError("confirmation seed result version drift")
    ...
    for seed in expected_seeds:
        ...
        seed_pairs = int(
            result["label_blind_pairwise_orientation_certificate_coverage"]
        )
    ...
    if full_coverage < 0 or full_coverage > 32:
        raise RuntimeError("confirmation full-history coverage outside frozen bounds")
    if pair_coverage < 0 or pair_coverage > 192:
        raise RuntimeError("confirmation pair coverage outside frozen bounds")
    if pair_coverage + ambiguous_pairs != 192 and not invalid:
        raise RuntimeError("valid confirmation pair accounting does not sum to 192")

    classification = _classification(full_coverage, invalid)
    aggregate = {
        "selection_version": (
            "chronotrace-pairwise-multi-witness-confirmation-selection-v2"
        ),
        ...
        "interpretation": (
            "Frozen label-blind 32-case confirmation interpreted exactly under the "
            "preregistered coverage thresholds; no post-launch method adaptation is applied."
        ),
    }
