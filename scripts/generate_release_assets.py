...
CONFIRMATION_LOCK = ROOT / "configs/chronotrace_pairwise_multi_witness_confirmation_v3.lock.json"
...
def _results_svg(selection: dict[str, Any], seeds: tuple[str, ...]) -> str:
    ...
    y = 385
    for seed in seeds:
        result = selection["per_seed"][seed]
...
def _pipeline_svg() -> str:
    ...
    boxes = []
    for x, title, line1, line2 in labels:
        boxes.append(
            f'<rect x="{x}" y="118" width="190" height="126" rx="16" class="box"/>'
            f'<text x="{x + 18}" y="158" class="boxtitle">{title}</text>'
            f'<text x="{x + 18}" y="190" class="bodytext">{line1}</text>'
            f'<text x="{x + 18}" y="214" class="bodytext">{line2}</text>'
        )
    arrows = []
...
<style>
 ...
 .bodytext {{ font-size: 14px; fill: #526070; }} .arrow {{ stroke: #3159c8; stroke-width: 3; fill: none; }}
 .tag {{ font-size: 15px; font-weight: 650; fill: #3159c8; }}
</style>
...
def _matrix(selection: dict[str, Any], seeds: tuple[str, ...]) -> str:
...
def _expected_files(selection: dict[str, Any], seeds: tuple[str, ...]) -> dict[Path, str]:
    return {
        ROOT / "assets/chronotrace-results.svg": _results_svg(selection, seeds),
        ROOT / "assets/chronotrace-pipeline.svg": _pipeline_svg(),
        ROOT / "paper/generated/results_macros.tex": _macros(selection),
        ROOT / "paper/figures/confirmation_matrix.tex": _matrix(selection, seeds),
    }
...
def main() -> int:
    ...
    confirmation_lock = _load(CONFIRMATION_LOCK)
    seeds = tuple(str(int(value)) for value in confirmation_lock["fresh_heldout_seeds"])
    files = _expected_files(selection, seeds)
    if args.write:
        _write(files)
        return 0
    return _check(files)
